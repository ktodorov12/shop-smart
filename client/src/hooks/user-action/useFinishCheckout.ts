...
export default function useFinishCheckout(handleCloseBag: () => void) {
  ...
  async function finishChekout() {
    ...
    try {
      const reducedProducts = addedToBag.reduce((acc: ProdInBag[], obj) => {
        ...
        existing.sizes.forEach((size) => {
          ...
          size.amount = String(reduced);
        });

        return acc;
      }, []);

      const promises = reducedProducts.map((prod): Promise<ProdInBag> => {
