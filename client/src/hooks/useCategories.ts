...
export default function useCategories(initialValues: Category[]) {
  const [categoriesStored, setCategoriesStored] = useState(initialValues);

  useEffect(() => {
    //Prone to bugs if data is changed in different browser;
    //TODO fix it;
    const isStored: Category[] | undefined = getSessionData("categories");
    if (isStored) {
  ...
  async function updateSublistAmount(categoryId: string, sublistName: string, operationType: string) {
