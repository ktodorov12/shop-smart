...
export default function Profile() {
  ...
  if (!user) {
    navigate("/login");
    throw new Error("You should login in order to see your profile!");
  }

  const callback = useCallback(() => getProductsProfile(user._id), [user._id]);
  const { values: products, isLoading } = useFetch<ProdData[]>(initialData, callback);
